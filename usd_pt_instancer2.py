...
#        print(np.any(np.abs(np.diff([,v2],axis=1))) == 1)
        ...
        ins_index.data.foreach_set("value", indicies[ic])
        blo_index.data.foreach_set("value", id[indicies[ic]])
        nbt_index.data.foreach_set("value", sub_id[ic])
...
def create_usd_collection(paths):
    ...
    if not bool(D.objects.get(file_name +' Blocks')):
        ...
        for obj in D.objects:
            if obj.name in path and not 'Block' in obj.name or 'Blocks' in obj.name:
        ...
        for i,obj in enumerate(block_objs):
            pre_loc = Matrix.Translation((row,0,column))
            try:
